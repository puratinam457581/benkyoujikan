...
- **各フェーズ完了時にコミットを作る。** 動作確認はユーザー自身が行う。
- **push は毎回の指示なしで行ってよい(2026-09-14 ユーザー明言)。** コミットしたら続けて `git push` する。ただし force push・履歴の書き換え・他ブランチへの操作は対象外(引き続き明示指示が要る)。
...
## 大幅アップデート: 学習管理システム連携(2026-09-07〜)

このアプリを、外部の「学習計画AI(Claude Project)」への**データ提供元**にする方向へ拡張。
計画立案・カレンダー参照・リマインダー登録はこのアプリの担当外(Claude アプリ側)。
参照ドキュメントはユーザー管理(学習管理システム_全体設計.md / 学習フェーズ.md /
過去ログ出力テンプレート.md)。詳細な設計判断は Obsidian の Code/Projects/benkyoujikan.md 参照。

- [x] 教材マスタ `meta/materials`(src/data/master.js): 教科ごとの教材台帳。完了フラグ。
      記録0hの教材も一覧化。起動時に records/tags から非破壊で取り込み(reconcileMaster)。
      「学習フェーズの教材を一括登録」ボタン(設定画面)
- [x] `meta/appConfig.phaseStart`: 現在フェーズ開始日(グローバル1つ、既定は今月1日)
- [x] 過去ログ出力(src/utils/exportLog.js, src/screens/ExportScreen.jsx): 過去ログテンプレ準拠。
      全期間累計 / 現在フェーズ累計 / 直近7日間の推移 / 教材の完了状況 / 大学の締切(「特になし」固定) /
      前日の日記。対象は前日まで。コピー / 共有(.mdファイル)ボタン。タブ「出力」(全6タブ)
- [x] 設定: 教材マスタ管理(MaterialStylesSection を刷新)、現在フェーズ開始日、
      「全データ削除」ボタン(記録・タグ・スタイル・マスタ・設定・日記を消す。confirm+入力の二段階確認)
- [x] 日記機能(任意記入。src/data/diary.js, src/components/DiaryCard.jsx): `users/{uid}/diary/{date}`
      コレクション。ホーム画面(今日)とカレンダーの日別詳細(選択日)から書ける。空で保存すると削除。
      前日ぶんが過去ログ出力の「前日の日記」に載る
- [x] 進捗・残り分量(任意。教材ごとに単位(ページ/レッスン/回など自由入力)・総量を設定)。
      現在地点は**記録(RecordModal)を保存するたびに任意で更新**できる(record.progress、
      src/data/records.js)。記録のたびに入力を強制はしない(空欄なら進捗データなし)。
      設定画面(MaterialStylesSection の ProgressEditor)は単位/総量の設定と、記録し忘れ等で
      ズレたときの基準値の補正(master.js の item.baseline)専用。現在地点・残り時間の算出は
      utils/progress.js の materialProgress()(基準値 + 記録の progress 値を時系列に並べ、
      直近7日以内のペース、無ければ直近2点のペースから残り時間を見積もる)。
      過去ログ出力の「教材の完了状況」に (現在/総量、残り、残り時間目安) を付記

記録モーダルの教材候補はこの教材マスタ(選択中の教科ぶん)から出す。活動内容は任意項目に変更
(出力は教科+教材までしか使わないため)。教科候補は 英語/数学/情報/大学 の基本セット + マスタ + 履歴。

### 2026-09-14 改善

- ScreenScaffold の下部余白を `pb-28`→`pb-20` に縮小(FABに隠れない最小限は概算62px、
  変更後80pxで運用。※この回はブラウザでの実機/実機相当の見た目確認ができておらず、
  数値上の見積もりのみ。実機で違和感があれば要調整)
- 過去ログ出力に「直近7日間の推移」(日ごとの合計時間の表)を追加
- 過去ログ出力に「前日の日記」を追加(日記機能とセット)
- 教材の進捗・残り分量(任意)を実装。設計相談を経て、現在地点は記録のたびに任意で更新する方式に変更
  (設定画面だけの更新案から変更。「毎回勉強すると進捗も変わる」というユーザーの指摘のため)。
  単位は自由入力のまま(ページ数に統一する案は、mikan/でる問1000/APG4b/公式問題集など
  ページで管理しづらい教材があるため不採用)。残り時間は実績ペースの自動算出のみ

### 2026-09-14 UI整理(機能は変えず、重複と不統一の解消)

「機能がごちゃごちゃしている」というフィードバックを受けた全体整理。**機能の追加・削除はしていない。**

- **タブを6→5に**: 「総計」+「比較」→「分析」タブへ統合(AnalysisScreen が中で累計/比較を切替)。
  どちらも集計画面で教科別ドーナツが重複していたため。TotalScreen/CompareScreen は
  ScreenScaffold を外した「ビュー」に変更(ファイル名は据え置き)
- **ホームの導線カード4枚 → 1枚**: 総計/カレンダー/比較はタブバーと行き先が完全に重複していたので削除。
  毎朝使う「学習計画用のログを出力」だけ残した
- **テーマ切替をヘッダーから削除し設定に集約**(同じ操作が2か所にあった)。
  ヘッダーは「アプリ名 / 画面名」だけにし、画面内の重複タイトル(ScreenScaffold の title)を廃止
- **`Section` コンポーネント新設**(src/components/Section.jsx): 見出しをパネルの外に出し、
  見出しの字体(field-label)とパネル余白(px-4 py-4)を1か所に統一。折りたたみ(collapsible)と
  警告色(tone='alert')にも対応。設定の「教材」「データの全削除」は既定で畳む
- **`.segmented` クラス新設**(index.css): 累計/比較・日次/週次/月次・教科別/教材別/活動内容別・
  ダーク/ライトの切替UIを、チップの羅列から統一したセグメント表示に
- カレンダーの月ナビと「今月に戻る」をカレンダー本体と同じパネルに統合(浮いた行をなくした)
- 比較のサマリーと棒グラフを1枚のパネルに統合。設定のテーマ選択アイコンの左右入れ違いを修正
- ※ この回もブラウザでの実機/実機相当の見た目確認はできておらず、ビルド通過と静的レビューのみ

### 2026-09-14 タブバー下の黒帯を修正(根本原因はCSSではなくメタタグ)

「画面下部の余白」の続報。pb-28→pb-20、height:100dvh、#root の position:fixed と
3回CSS側で試したが実機で一切変化なし。ユーザーからの「PWAの仕様では」という指摘で
WebSearch/WebFetchして根本原因を特定 → 詳細は Obsidian [[ios-pwa-bottom-safe-area]]。

**原因**: `apple-mobile-web-app-status-bar-style: black-translucent` を指定すると、iOS は
インストール済みPWAのWebViewを「画面高−上部セーフエリア」で上端固定配置する。
下端のセーフエリア帯はWebViewの箱の外(OSが塗る黒)になり、ページ内のCSS/JSからは
到達も計測もできない。だからCSSを何度直しても変わらなかった。

**修正**: `black`(不透明)に変更(commit 0023d67)。WebViewが画面いっぱいに配置され、
タブバーの `env(safe-area-inset-bottom)` が実際の画面端に効くようになる。
**実機で解消を確認済み**(ホーム画面アイコンの削除→再追加が必須。iOSがインストール時に
メタタグを固定するため、SW更新だけでは反映されない)。
