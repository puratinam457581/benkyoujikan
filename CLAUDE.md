...
- [x] 進捗・残り分量(任意。src/data/master.js の unit/total/current/progressHistory):
      教材ごとに単位(章/問/%など自由入力)・総量・現在地点を設定。現在地点は絶対値で上書き入力
      (設定画面 MaterialStylesSection の ProgressEditor)。残り時間は本人の見積もりではなく、
      現在地点を2回以上更新した間の実績ペース(分/単位)から自動算出(estimateRemainingMinutes)。
...
- 教材の進捗・残り分量(任意)を実装。単位は自由入力、現在地点は絶対値上書き、
  残り時間は実績ペースの自動算出のみ(ユーザーの見積もり値は併用しない)。ユーザーとの設計相談で決定
